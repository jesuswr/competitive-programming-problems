// You can see costs of edges as gains if you multiply them by -1
...
typedef int tf;
typedef int tc;
const tf INFFLOW = 1e6;
const tc INFCOST = 1e6;
struct MCF {
    int n;
...
int main() {
    ...
    FOR(i, 0, 26) {
        mcf.add_edge(s, i, cnt[i], 0);
    }
    FOR(i, 0, n >> 1) {
        mcf.add_edge(26 + i, t, 2, 0);
    }
    FOR(i, 0, 26) {
        FOR(j, 0, n >> 1) {
            int cst;
            if ('a' + i == a[j] && a[j] == a[n - 1 - j])
                cst = -max(b[j], b[n - 1 - j]);
            else if ('a' + i == a[j])
                cst = -b[j];
            else if ('a' + i == a[n - 1 - j])
