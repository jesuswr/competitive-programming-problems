...
int main() {
    rll(N, M);
    // vector<ll> dp(M); dp[M - 1] = 1;
    // FOR(j, 0, N) {
    //     vector<ll> new_dp(M);
    //     FOR(i, 0, M - 1) new_dp[i] = dp[i + 1];
    //     new_dp[M - 1] = dp[M - 1] + dp[0];
    //     printf("%d : %lld\n", j, new_dp[M - 1]);
    //     dp = new_dp;
    // }

    Matrix m(0);
    m.mat[0][M - 1] = m.mat[M - 1][M - 1] = 1;
    FOR(i, 0, M - 1) m.mat[i + 1][i] = 1;
    m = bin_exp(m, N);

    // FOR(i, 0, M) {
    //     FOR(j, 0, M) printf("%lld ", m.mat[i][j]);
    //     printf("\n");
    // }
